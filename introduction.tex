...
% ==== Title ====
\title{Deep Generative Models: A Visual Overview (2025)}
\subtitle{Taxonomy, Intuitions, Equations, and Trade-offs}
\author{\\Your Name \\ University Course Lecture}
\date{\today}
...
\begin{itemize}
  ...
  \item Compare the five major families: Autoregressive, VAE, GAN, Normalizing Flows, Diffusion/Score.
...
\begin{frame}{Prologue: The Bitter Lesson (Sutton)}
\small
...
\begin{itemize}
  ...
  \item Evaluate by out-of-distribution generalization and transfer, not by fitting handcrafted cases.
\end{itemize}
...
\begin{frame}{Definition \\ \small and Why It Matters}
...
\centering
\begin{adjustbox}{max size={\linewidth}{.70\textheight}}\begin{tikzpicture}[node distance=1.0cm, every node/.style={font=\small}]
  % Axes
...
\centering
\begin{adjustbox}{max size={\linewidth}{.70\textheight}}\begin{tikzpicture}[node distance=0.8cm]
...
\begin{frame}{Idea and Objective}
\centering
...
\begin{frame}{Invertible Transformations}
\centering
\begin{adjustbox}{max size={\linewidth}{.50\textheight}}
...
\begin{frame}{Forward and Reverse Processes}
\centering
\begin{adjustbox}{max size={\linewidth}{.50\textheight}}
...
\begin{itemize}
  ...
  -\dfrac{\mathbf{x}_t-\sqrt{\bar\alpha_t}\,\mathbf{x}_0}{1-\bar\alpha_t}$,
  and $s_\theta \approx -\dfrac{1}{\sqrt{1-\bar\alpha_t}}\,\varepsilon_\theta$.
\end{itemize}
\end{frame}
...
\toprule
\textbf{Family} & \textbf{Likelihood} & \textbf{Quality} & \textbf{Speed (sample)} & \textbf{Control} & \textbf{Stability} & \textbf{Notes} \\
\midrule
Autoregressive & \cmark & \cmark (text/code) & \xmark & \cmark & \cmark & slow sequential; great for discrete \\
VAE & bound & mixed & \cmark & \cmark & \cmark & scalable; useful latents \\
GAN & \xmark & \cmark & \cmark & mixed & mixed & sharp images; mode collapse risk \\
Flows & \cmark & mixed & mixed & \cmark & \cmark & exact log-lik; invertible design \\
Diffusion & surrogate & \cmark & mixed$\to$\cmark & \cmark & \cmark & top-tier visuals; few-step via distillation \\
\bottomrule
\end{tabular}
\end{frame}
...
\begin{frame}{AR vs. Diffusion: Equations Side-by-Side}
\small
\begin{columns}[T]
  \begin{column}{0.48\linewidth}
    ...
    \begin{align*}
      ...
      \mathcal{L}_{\text{NLL}} &= -\sum_{i=1}^{D} \log p_\theta(x_i\mid x_{<i}).
    \end{align*}
  \end{column}
  \begin{column}{0.48\linewidth}
    \textbf{Diffusion (noise-prediction)}
    \begin{align*}
      \mathcal{L} &= \mathbb{E}_{\mathbf{x},t,\varepsilon}\big[\,\lVert \varepsilon - \varepsilon_\theta(\mathbf{x}_t,t) \rVert^2\big],\\
      &\text{with}\; \mathbf{x}_t = \sqrt{\bar\alpha_t}\,\mathbf{x}_0 + \sqrt{1-\bar\alpha_t}\,\varepsilon.
    \end{align*}
  \end{column}
\end{columns}
\end{frame}

\begin{frame}{Flow Matching (for intuition)}
\small
...
\begin{frame}{Placeholders for Your Course Figures}
\centering
\placeholderfig[0.6em]
\placeholderfig
\end{frame}
