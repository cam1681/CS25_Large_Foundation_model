...
Given a latent variable model $p_\theta(x,z) = p_\theta(x|z)p(z)$, we want to:
\begin{itemize}
...
\begin{equation}
\ELBO(x; \theta, \phi) = \EE_{q_\phi(z|x)}[\log p_\theta(x|z)] - \KL(q_\phi(z|x) \| p(z))
\end{equation}
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textcolor{red}{Regularization term:} $-\KL(q_\phi(z|x) \| p(z))$
    \begin{itemize}
        \item Encourages $q_\phi(z|x)$ to be close to prior $p(z)$
        \item Prevents overfitting to training data
    \end{itemize}
\end{enumerate}
\end{frame}

\section{A mathematical route}
\begin{frame}{From Jensen to the ELBO}
For any variational posterior \(q_\phi(z\mid x)\),
\begin{align*}
\log p_\theta(x)
&= \log \int q_\phi(z\mid x)\,\frac{p_\theta(x,z)}{q_\phi(z\mid x)}\,dz \\
&\ge \E_{q_\phi} \Big[ \log p_\theta(x,z) - \log q_\phi(z\mid x) \Big] \quad (\text{Jensen}) \\
&= \underbrace{\E_{q_\phi}\big[\log p_\theta(x\mid z)\big]}_{\text{reconstruction}}
- \underbrace{\KL\big(q_\phi(z\mid x)\,\|\,p(z)\big)}_{\text{regularization}} \\
&\eqqcolon \mathcal L(\theta,\phi;x)\quad \text{(ELBO)}.
\end{align*}
Moreover, the identity
\[
\log p_\theta(x) = \mathcal L(\theta,\phi;x) + \KL\big(q_\phi(z\mid x)\,\|\,p_\theta(z\mid x)\big)
\]
shows \(\mathcal L\) is a lower bound and is tight iff \(q_\phi(z\mid x)=p_\theta(z\mid x)\).
\end{frame}


\begin{frame}{Two Equivalent Forms of the ELBO}
\textbf{(1) Recon - KL form}
\[
\mathcal L = \E_{q_\phi}\big[\log p_\theta(x\mid z)\big] - \KL\big(q_\phi(z\mid x)\,\|\,p(z)\big).
\]
\textbf{(2) Variational free energy}
\[
-\mathcal L = \underbrace{\KL\big(q_\phi(z\mid x)\,\|\,p_\theta(z\mid x)\big)}_{\text{inference gap}}
- \log p_\theta(x).
\]
At fixed \(\theta\), the \(\mathcal L\)-maximizer is the true posterior.
\end{frame}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Prior:} $p(z) = \mathcal{N}(z; 0, I)$
\end{itemize}
\end{frame}
...
\begin{remark}
    The reparameterization trick also leans the lights on the diffusion model.
\end{remark}
...
Expanding the ELBO:
\begin{equation}
\mathcal{L}(\theta, \phi) = \sum_{i=1}^N \left[ \EE_{q_\phi(z|x^{(i)})}[\log p_\theta(x^{(i)}|z)] - \KL(q_\phi(z|x^{(i)}) \| p(z)) \right]
\end{equation}
...
\begin{enumerate}
    ...
    \item Compute KL divergence: $\KL(q_\phi(z|x^{(i)}) \| p(z))$
...
\begin{itemize}
    ...
    \item Simple, interpretable loss function
\end{itemize}
\end{frame}
