...
% ===================== Frame 2 =====================
\begin{frame}{Why the Posterior is a Problem? How about $p_\theta(x)$ not $\log p_\theta(x)$?}
...
Apply Jensen's inequality (log is concave, s.t. $E_p[\log x] \ge \log E_p[x]$):
\begin{align*}
\log p_\theta(x)
...
\begin{frame}{Decomposition: $\log p_\theta(x)$, ELBO, and Posterior KL}
...
\begin{frame}{Rethinking the VAE from the generative perspective}
    We have $p_data(x)$ and we want to learn a model $p_\theta(x)$ that can generate realistic samples from the easily sampling latent variable $z$.
    \begin{itemize}
        ...
        \item We can approximate the posterior $p_\theta(z|x)$ with a neural network $q_\phi(z|x)$.
        \[\nabla_\theta \log p_\theta(x) = \mathbb{E}_{q_\phi(z|x)}[\nabla_\theta \log p_\theta(x|z)]\]
        \item Compute the KL divergence between $q_\phi(z|x)$ and $p_\theta(z|x)$:
        \[\mathrm{KL}(q_\phi(z|x) \| p_\theta(z|x)) = \log p_\theta(x) - ELBO(x; \theta, \phi)\]
    \end{itemize}

\begin{remark}
    Why we did not directly optimize $p_\theta(x)$?
\end{remark}
\end{frame}
