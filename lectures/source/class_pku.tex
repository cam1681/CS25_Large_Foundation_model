...
\section{Understand diffusion models from a mathematical viewpoint}

\begin{frame}{Core problem in generative modeling: Generate samples from given distribution}
Given a distribution $p(x)$, how to sample samples from the distribution?
\begin{itemize}
    \item $p(x)$ is known, sample from $p(x)$
    \item $p(x)$ is unknown, estimate $p(x)$ from data and sample from $p(x)$
\end{itemize}

\vspace{1em}

$\square$ When $p(x)$ is known:
\begin{itemize}
    \item Uniform distribution: easy sampling 
    \item Mixture Gaussian: first determine which Gaussian to sample and then sample from that Gaussian
    \item A general probability distribution?
    \begin{itemize}
...
\begin{frame}{Molecular Dynamics (MD) is one kind of special sampling methods}
$\square$ Continuous form of the Langevin MCMC:
\begin{itemize}
    \item $\Delta t\to 0$
    \begin{equation}
...
$\square$ MD is one kind of special sampling methods:
\begin{itemize}
...
\begin{frame}{The relationship between the clean data $x$ and the noised data $\Tilde{x}$}

$\square$ From $x$ to $\Tilde{x}$: adding noise by Gaussian transition kernel
\begin{equation}
...
$\square$ Questions arise: how to remove the noise from $\Tilde{x}$ to get $x$? (Denoising)
\end{frame}
...
\begin{enumerate}
    \item Score model 
    \begin{equation}
        Loss_{score} = \|s_\theta(\Tilde{x}) - \nabla_{\Tilde{x}}\log p(\Tilde{x}|x)\|^2 = \|s_\theta(\Tilde{x}) - \frac{\epsilon}{\sigma}\|^2;
    \end{equation}
...
\begin{enumerate}
    ...
    \end{equation}
    where the mean value $\Tilde{\alpha}_t x= x(0)$  and standard deviation $\Tilde{\sigma}_t = \sqrt{\sigma^2(t)-\sigma^2(0)}\approx \sigma(t)$ in VE. VP is similar with the mean and standard deviation from its transition kernel;
...
\begin{frame}{Trajectories and Probability Transition}
    \begin{figure}
        \includegraphics[width=0.38\textwidth]{../figures/flow2.png}\\
