...
\textbf{Key insight}: This equation ensures probability conservation as particles flow according to the velocity field.

\textbf{Physical meaning}:
\begin{itemize}
    \item \(\partial_t p(\vx,t)\): Rate of change of probability density
    \item \(\grad\cdot\big(p(\vx,t)\,\mathbf{v}(\vx,t)\big)\): Divergence of probability flux
    \item The equation states: "Change in density = Net flow in/out"
\end{itemize}

\textbf{Connection to Flow Matching}: This equation is the fundamental constraint that any learned velocity field must satisfy to properly transport probability distributions.
\end{frame}
...
\textbf{Mathematical insight}: This derivation shows that the conditional velocity field has a simple closed-form expression for Gaussian paths.

\textbf{Practical significance}: This analytic form enables efficient training without numerical differentiation or score estimation.
\end{frame}
...
\textbf{Practical implications}: This optimality result guarantees that Flow Matching will converge to the best possible velocity field for the chosen conditional paths.
\end{frame}
...
\textbf{Theoretical guarantee}: Under mild conditions, the generated samples will follow the target distribution \(p_0\).
\end{frame}
