...
\tikzset{
    s2-math-matrix/.style={
        ...
        right delimiter={]},
        nodes={
            minimum size=0.8cm, % Compact size
            text height=1.5ex,
            text depth=0.5ex,
            draw=gray!20,
            fill=gray!5,
...
\begin{frame}{Matrix Multiplication: Row Picture}
    ...
    % Formula Placeholder (Internal spacing tightener)
    ...
    % Step 4: Display formula for Row 1
    \only<4>{
    ...
    % Step 6: Highlight Row 2 of A
    \only<6-9>{
        \node[s1-highlight-box, fit=(A-2-1)(A-2-2)] (rowA2) {};
    }

    % Step 7-9: Extract coefficients
    \only<7-9>{
    ...
    % Step 8: Display formula for Row 2
    \only<8>{
...
\begin{frame}{Attention Construction: Alignment \& Masking}
    ...
    % === Bounding Box Fix ===
    % Centers the Q-K-S diagram
    \useasboundingbox (-0.5, -3.0) rectangle (8.5, 2.0);
    ...
    \node[left=0.1cm of Q, font=\footnotesize, align=right, color=gray] {
        Token 1\\[1.5em]Token 2\\[0.8em]\\Token 3
    };

    \node[right=0.15cm of Q] (times) {$\times$};

    % --- 2. Key Matrix Transposed (K^T) ---
    ...
    % === Step 2: Calculate Score (1,2) -> FUTURE! ===
    \only<3>{
    ...
    % === Step 4: Row 2 Calculation & Masking ===
    \only<5>{
...
\begin{frame}{Attention Mechanism: Linear Combination of Values}
    ...
    % === SCENARIO 1: Row 1 Calculation ===
    % Step 2: Highlight Row 1 of A
    \only<2-5>{
        \node[s3-highlight-box, fit=(Attn-1-1)(Attn-1-2)] (rowA1) {};
    }
    
    % Step 3-5: Coefficients fly to V
    \only<3-5>{
        ...
        % Highlight V rows (Unified Style)
    ...
    % === SCENARIO 2: Row 2 Calculation ===
    % Step 6: Highlight Row 2 of A
    \only<6-9>{
        \node[s3-highlight-box, fit=(Attn-2-1)(Attn-2-2)] (rowA2) {};
    }

    % Step 7-9: Coefficients fly
    \only<7-9>{
